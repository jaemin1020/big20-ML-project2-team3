...
def train_and_evaluate_regressor(
    ...
    - hyperopt_search(...) 로 찾은 파라미터를 사용하여
      최종 모델을 학습하고, 검증 세트에서 회귀 메트릭을 계산.
    - 모델은 pickle(.pkl)로 ../models 에 저장.
    - 메트릭/설정은 json으로 ../results 에 저장.
    - target_is_log1p=True 인 경우:
        y_train, y_test 가 log1p(price) 스케일이라고 가정하고
        원래 price 스케일로 복원하여 RMSE/MAE/R2/RMSLE 를 계산.

    Parameters
    ----------
    model_class : class
        sklearn / xgboost / lightgbm 등의 회귀 모델 클래스 (예: LGBMRegressor)
    params : dict
        하이퍼파라미터 딕셔너리
    X_train, y_train, X_test, y_test : array-like or sparse matrix
        학습/검증 데이터
    save_model : bool
        True 이면 모델 pickle 저장
    save_model_path : str
        모델 저장 경로 (기본: ../models/)
    save_result_path : str
        결과 json 저장 경로 (기본: ../results/)
    verbose : bool
        True 이면 로그 출력
    ...
    dict
        {
            'model': 학습된 모델 객체,
            'metrics': 메트릭 딕셔너리,
            'model_path': 모델 저장 경로 (옵션),
            'result_path': 결과 json 저장 경로 (옵션),
            'fit_time': 학습 시간(초)
        }
    """
    model_name = model_class.__name__
