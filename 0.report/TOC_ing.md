...
#### 3.5.1 최종 평가 지표 분석
- Recall, Precision, F1 Score 결과 분석
- ROC-AUC 및 F2-Score 결과 제시
...
- 혼동 행렬 시각화
- Precision-Recall Curve를 통한 임계값 최적화
- False Negative 발생 원인 분석

### 3.6 결론 및 제언 - kjh

#### 3.6.1 주요 결과 요약
- 최종 선정된 앙상블 모델 및 최종 성능

#### 3.6.2 프로젝트 한계점 분석
- 데이터 측면 (익명화된 피처, 시간적 순서 반영 부족)
- 모델 측면 (정밀도와 재현율 간의 Trade-off)

#### 3.6.3 개선 방안 및 향후 계획
- 전처리/샘플링 단계별 모델 성능 비교
- 임계값 조정 자동화 및 Recall 극대화 방안
- 실시간 탐지 시스템 구축을 위한 모델 경량화
