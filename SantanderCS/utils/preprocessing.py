...
def remove_zero_columns(df, threshold_rate=0.99, save_report=True):
    ...
    print(f'\n{"="*102}')
    print(
        f"Zero Value Analysis (Threshold: {threshold_rate*100}% = {int(threshold):,} rows)"
    )
    print("=" * 102)
    ...
    print("=" * 102)
    print(
        summary_df.sort_values(by="zero_count", ascending=False).to_string(index=False)
    )
    ...
    if save_report and len(remove_cols) > 0:
        ...
        for i, col in enumerate(remove_cols, 1):
            ...
            print(
                f"{i:3d}. {col:20s} - zero_count: {col_info['zero_count']:>10,} "
